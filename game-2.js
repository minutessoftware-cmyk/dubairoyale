...
const ZONES = [
  ...
  { name: "Burj Al Arab",   x: 35,   z: 85,   color: 0xEAF3F7 },
];
...
function buildLandmarks() {
  // Burj Khalifa — central tapering spire made of stacked tiers, topped with an antenna mast
  ...
  for (let i = 0; i < 3; i++) {
    ...
    m.castShadow = true;
    scene.add(m);
    topOfTiers = Math.max(topOfTiers, h);
  }
  const antennaMat = new THREE.MeshStandardMaterial({ color: 0xD8D2C4, roughness: 0.4, metalness: 0.5 });
  const antenna = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.5, 20, 6), antennaMat);
  antenna.position.set(0, topOfTiers + 10, 0);
...
function buildBurjAlArab() {
  ...
  const island = new THREE.Mesh(new THREE.CylinderGeometry(15, 16, 1.2, 24), islandMat);
  island.position.set(zx, 0.6, zz);
  island.receiveShadow = true;
  scene.add(island);

  const bridgeMat = new THREE.MeshStandardMaterial({ color: 0xC9A15C, roughness: 1 });
  const bridge = new THREE.Mesh(new THREE.BoxGeometry(6, 1, 26), bridgeMat);
  ...
  for (let i = 0; i < segs; i++) {
    const t = i / (segs - 1);
    const width = 16 * (1 - t * 0.85);
    const depth = 3.5 * (1 - t * 0.5);
    const curveOffset = Math.sin(t * Math.PI * 0.5) * 8;
    const seg = new THREE.Mesh(new THREE.BoxGeometry(width, segH, depth), sailMat);
    seg.position.set(zx, 3 + i * segH * 0.92, zz - 6 + curveOffset);
    seg.castShadow = seg.receiveShadow = true;
    scene.add(seg);
    if (i === 0) addObstacleBox(seg, zx, zz, width / 2, depth / 2 + 2);
  }

  // helipad disc partway up, and a warning beacon on the mast at the top
  const helipad = new THREE.Mesh(new THREE.CylinderGeometry(4, 4, 0.4, 16),
    new THREE.MeshStandardMaterial({ color: 0xE4483C, roughness: 0.6 }));
  helipad.position.set(zx + 6, segH * 6, zz - 2);
  ...
  scene.add(mast);
  const beacon = new THREE.Mesh(new THREE.SphereGeometry(0.4, 8, 8), new THREE.MeshBasicMaterial({ color: 0xE4483C }));
  beacon.position.set(zx, mastTopY + 3.2, zz + 2);
  scene.add(beacon);
}
